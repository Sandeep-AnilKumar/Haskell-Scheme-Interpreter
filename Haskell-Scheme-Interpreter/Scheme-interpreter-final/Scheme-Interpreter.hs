...
-- implementation for functions. Functions consists of parameters, variable number of arguments, body and closure.
	     | Func { params :: [String], vararg :: (Maybe String),
         	      body :: [LispVal], closure :: Env }

--For IO operations like loading a file and reading it.
	     |IOFunc ([LispVal] -> IOThrowsError LispVal)

-- Constructor for port datatype in Scheme. In haskell Handle has the same notion as port.
	     | Port Handle
...
-- implementation for primitive functions and user defined functions.
showVal (PrimitiveFunc _) = "<primitive>"

-- When a function like "(define (f x y) (+ x y))" is passed, this will return "(lambda ("x" "y")...)"
...
-- ShowValue for IO ports and primitives.
...
-- defintion for evaluating different user defined functions.
...
-- load can introduce bindings into the environment, and since apply does not take any env arguments, load is defined as a special function.
eval env (List [Atom "load", String filename]) = 
     load filename >>= liftM last . mapM (eval env)

-- here we are implementing the function call. we take the arguments and evaluate it and then map it by applying to func.
eval env (List (function : args)) = do
     func <- eval env function
     argVals <- mapM (eval env) args
     apply func argVals

-- if evaluation of expr results in error.
...
-- for user defined functions first it will check whether the parameters of the function and the arguments are of same size, and if there are no
-- variable number of arguments. If this is true, then the params and args are zipped to assign values to params, and they are bounded in the env
-- later the bindVarArgs binds the remaninig variable number of args and evaluates the body.
...
-- apply function for IO functions.
...
-- Since the types signtaures of primitives and ioPrimitives are different, we will be using a different list for IO operations.
...
-- applyProc destructures arguments in the form the apply function expects.
applyProc :: [LispVal] -> IOThrowsError LispVal
applyProc [func, List args] = apply func args
applyProc (func : args)     = apply func args


-- wraps the openFile Haskell Function. modes can be read write and IO.
makePort :: IOMode -> [LispVal] -> IOThrowsError LispVal
makePort mode [String filename] = liftM Port $ liftIO $ openFile filename mode


-- wraps the hClose Haskell Function.
closePort :: [LispVal] -> IOThrowsError LispVal
closePort [Port port] = liftIO $ hClose port >> (return $ Bool True)
closePort _           = return $ Bool False


-- wraps the hGetLine Haskell Function and send the expr to the ParseExpr, which turns the result into LispValue suitable for Scheme.
readProc :: [LispVal] -> IOThrowsError LispVal
readProc []          = readProc [Port stdin]
readProc [Port port] = (liftIO $ hGetLine port) >>= liftThrows . readExpr


-- wraps the hPrint Haskell Function. Since hPrint takes a value of flow, we do nothave to explicitly call show. Since most functions do so,
-- this is the reason we converted LispValue to a type of show.
writeProc :: [LispVal] -> IOThrowsError LispVal
writeProc [obj]            = writeProc [obj, Port stdout]
writeProc [obj, Port port] = liftIO $ hPrint port obj >> (return $ Bool True)


-- wraps the readFile Haskell Function, reads the whole file into a string in the memory.
readContents :: [LispVal] -> IOThrowsError LispVal
readContents [String filename] = liftM String $ liftIO $ readFile filename


-- load is used in readAll, to read a file full of statements.
load :: String -> IOThrowsError [LispVal]
load filename = (liftIO $ readFile filename) >>= liftThrows . readExprList


-- readAll then wraps the return value with the String constructor.
...
-- readExprList and readExpr are specializations of readOrThrow. readExpr will be used in REPL to read expressions and readExprList is used
-- within load to read programs.
...
-- Since we're now storing primitives as regular values in variables, we have to bind them when the program starts up. This takes the initial null
-- environment, makes a bunch of name/value pairs consisting of PrimitiveFunc wrappers, and then binds the new pairs into the new environment.
...
-- During the start the environment is null. And the expr is evaluated. The runOne takes the file to execute and runs that program. Additional
-- command line arguments will be bounded into a list args within the scheme program.
runOne :: [String] -> IO ()
runOne args = do
