...
-- if-clause evaluator. pred is predicate, which will result in true or false on evaluation. If it is true conseq is evaluated, alt if it is false.
-- if predicate is not true or false, then error is thrown.
...
-- cond implementation, which is a flavor of how if-else ladder works in Java.
...
--case implementation, here the key is evaluated and every clause is checked if it is equal to the key. If none of the clauses match, then
-- else clause is executed. If else clause is not present then, the result will be unspecified.
